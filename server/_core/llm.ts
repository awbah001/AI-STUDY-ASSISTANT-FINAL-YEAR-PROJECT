...
const flattenMessageContent = (message: Message): string => {
...
const assertForgeApiKey = () => {
  if (!ENV.forgeApiKey) {
    throw new Error(
      "BUILT_IN_FORGE_API_KEY is not configured. Set USE_LOCAL_LLM=true (default) and run LM Studio, or configure Forge."
    );
  }
};
