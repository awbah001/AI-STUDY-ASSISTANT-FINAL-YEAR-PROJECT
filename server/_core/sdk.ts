...
class OAuthService {
  constructor(private client: ReturnType<typeof axios.create>) {
    if (!ENV.oAuthServerUrl) {
      console.log(
        "[OAuth] OAUTH_SERVER_URL not set — Manus OAuth disabled. Using local email/password sign-in."
      );
    } else {
...
class SDKServer {
  ...
  async verifySession(
    ...
    try {
      ...
      // For local auth we only need `openId` to resolve the user in DB.
      // Keep backward-compatible optional fields for older/newer token shapes.
